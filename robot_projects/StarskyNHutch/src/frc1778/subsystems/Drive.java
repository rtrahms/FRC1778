/**
 * Drive.java - controls the 4 or 2 jaguars that do
 * wheel driving.  Including dealing with all the dang jag modes.
 * position, speed and current are PID modes 
 * (the =JAG= does the PID on board)
 * remaining are NON-PID modes:
 * percentVBus is the usual -1..0..1 usually used.
 * voltage uses and absolute voltage number - rarely used.
 */
...
//      }
  ...
  */
   ...
   */
   ...
   /**
    * see if either position mode pid has reached desired spot that we 
    * set with Straight()=>SetLeftRight()
    */
    public boolean atPosition() { 
        try {
            System.out.println("lGet= " + l_Master.getX() + " rGet= " + r_Master.getX()); 
            jagShow(); 
            if (Math.abs(r_Master.getX() - r_Master.getPosition()) < 0.02) return true; 
            if (Math.abs(l_Master.getX() - l_Master.getPosition()) < 0.02) return true; 
        }
        catch (CANTimeoutException e) {}
        return false;
    }
  ...
  /**
   * put our jags into regular percent coast mode 
   * test one at a time so we don't grind nothin 
  * then put back in standard mode 
  * make sure a side moves in only ONE direction 
  * and that net console shows encoders reporting
  *expected signs
  */
    public void TEST() { 
        try {
